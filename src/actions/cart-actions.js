...
export const updateCartItem = async (cartId, sanityProductId, data) => {
...
export const syncCartWithUser = async (cartId = null) => {
  const { user } = await getCurrentSession();
  ...
  for (const item of existingAnonymousCart.items) {
    const existingItem = existingUserCart.items.find(
      (i) => item.sanityProductId === i.sanityProductId
    );
...
export const addWinningItemToCart = async (cartId, product) => {
  const cart = await getOrCreateCart(cartId);
